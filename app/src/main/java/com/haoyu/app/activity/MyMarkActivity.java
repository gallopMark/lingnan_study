...
public class MyMarkActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void shakeAssignment() {
        String url2 = Constants.OUTRT_NET + "/" + aid + "/study/unique_uid_" + uid + "/m/assignment/mark?assignment.id=" + assignmentId + "&assignmentRelationId=" + relationId;
        OkHttpClientManager.getAsyn(context, url2, new OkHttpClientManager.ResultCallback<CourseActivityListResult>() {
            @Override
            public void onError(Request request, Exception e) {
                hideTipDialog();
            }
